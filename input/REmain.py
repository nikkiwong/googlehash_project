...
def mutation_algorithm(number_of_caches, number_of_videos, list_cache, list_endpoint, video_ed_request, video_size_desc):
    ...
    # while x < 10:
    for cacheNum in range(0, number_of_caches):
    #iterates through the caches and mutates one random video per cache until it reaches the end of the list
    #repeats this process 10 times
        ...
        if list_cache[cacheNum].mutate_solution(n, video_size_desc[n]):
            ...
            if new_score > mutate_Max:
                ...
                print("MUTATED MAX", mutate_Max)
        # x += 1
    if len(parents)>8:
        #I have this condition here in case the parent list, when divided in half, does not give 4 samples.
        #if that is the case then we will get value error.
